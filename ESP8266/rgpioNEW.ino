...
void loop() {
  ...
  if (noBytes) {
    ...
    if (str.startsWith("Set/Dop:")) {

      String pinName = str.substring(8, str.indexOf("/", 8));
      String pinState = str.substring(str.indexOf("Value:") + 6);

      Serial.print("Set pinName=");
      Serial.print(pinName);
      Serial.print("Set pinState=");
      Serial.println(pinState);

      setDEVICE(pinName, pinState);
      UdpSendString(UdpReceive.remotePort(), serverIP, "OK");
    }

    if (str.startsWith("Get/Dip:") || str.startsWith("Get/Aip:")) {
      String pinName = str.substring(8);
      String pinValue;

      Serial.print("GET pinName=");
    ...
    if (str.equals("OK")) { }  // What is in fact the use of the server sending OK ??
